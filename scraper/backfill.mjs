...
async function scrapeRange(page, url, { start, end }) {
  ...
  for (const rec of seen.values()) {
    ...
    if (isRepostRecord(rec)) {
      ...
      // time, so it can't be bucketed by day directly. Anchor it to the
    ...
    if (ts < start) break; // scrolled past the target window; everything after is older still
    if (ts >= end) continue; // outside the window on the newer end
